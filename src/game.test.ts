...
test.afterEach(t => t.context.removeEventListeners());

test.serial("is created with correct starting state", t => {
    t.is(t.context.state.currentTurn, "white");
});

test.serial("move throws error when origin square is empty", t => {
    t.throws(() => t.context.move([0, 2], [0, 3]), { message: "Cannot move piece at [0,2]" });
});

test.serial("move throws error when piece is not owned by current player", t => {
    t.is(t.context.board.get([0, 6])?.colour, "black");
    t.throws(() => t.context.move([0, 6], [0, 5]), { message: "Cannot move piece at [0,6]" });
});

test.serial("move throws error when captured piece is owned by current player", t => {
    t.is(t.context.board.get([3, 1])?.colour, "white");
    t.throws(() => t.context.move([3, 0], [3, 1]), { message: "Cannot capture piece at [3,1]" });
});

test.serial("move throws error when captured piece is a king", t => {
...
test.serial("move throws error when one or more intermediate square is not empty unless piece is a knight", t => {
...
test.serial("move updates piece position and swaps current player", t => {
